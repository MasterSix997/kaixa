...
namespace kaixa::plugin::cmake::detail {
    namespace {
        Result<bool> inspect_regular_file(const std::filesystem::path& path) {
            ...
            if (failure) {
                return std::unexpected(error("cannot inspect `" + path.string() + "`: " + failure.message()));
            }
            if (!exists)
                return false;
        ...
        Result<std::filesystem::path> latest_index(const std::filesystem::path& reply) {
            ...
            if (failure) {
                return std::unexpected(error("cannot inspect CMake File API replies in `" + reply.string() + "`: " + failure.message()));
            }
            if (!exists)
                return {};
            ...
            if (failure) {
                return std::unexpected(error("cannot list CMake File API replies in `" + reply.string() + "`: " + failure.message()));
            }
        ...
        Result<bool> changed_after(const std::filesystem::path& input, const std::filesystem::file_time_type configured_at) {
            ...
            if (failure) {
                return std::unexpected(error("cannot read timestamp for `" + input.string() + "`: " + failure.message()));
            }
    ...
    Result<ActionState> configuration_state(
        ...
        if (failure) {
            return std::unexpected(error("cannot read CMake File API reply timestamp `" + index->string() + "`: " + failure.message()));
        }
