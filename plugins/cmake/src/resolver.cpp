    ...
    namespace {
        ...
        Result<std::string> dependency_integration(const DependencyIntegrationContext& context) {
            ...
            for (const PackageNode& package: context.graph.nodes()) {
                if (package.source && !package.directory.empty()) {
                    result += "  set(" + detail::source_variable(package.id) + " " + cmake_quote(package.directory) + ")\n";
                }
            }
        ...
        private:
            [[nodiscard]] Result<void> plan_route(const ConfiguredRoute& route, RoutePlanningContext& planning) const {
                ...
                if (context->project.generation == GenerationMode::export_project) {
                    std::vector<detail::PortableProject> portable_projects;
                    portable_projects.reserve(source_packages.size());
                    for (const PackageId id: source_packages)
                        portable_projects.push_back({id, projects[id.index]->source, !normal_source_visited[id.index]});

                    auto portable = detail::generate_portable_dependencies(graph, package, context->project, portable_projects);
                    if (!portable)
                        return std::unexpected(portable.error());

                    plan.generate({context->project.source / "KaixaDependencies.cmake", std::move(*portable)});
                }
