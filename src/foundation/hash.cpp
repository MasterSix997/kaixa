...
namespace kaixa {
        ...
        std::string digest(std::vector<std::uint8_t> bytes) {
            ...
            for (std::size_t offset = 0; offset < bytes.size(); offset += 64U) {
                ...
                for (std::size_t index = 0; index < words.size(); ++index) {
                    const std::uint32_t sigma_one = std::rotr(e, 6) ^ std::rotr(e, 11) ^ std::rotr(e, 25);
                    const std::uint32_t choice = (e & f) ^ (~e & g);
                    const std::uint32_t temporary_one = h + sigma_one + choice + constants[index] + words[index];
                    const std::uint32_t sigma_zero = std::rotr(a, 2) ^ std::rotr(a, 13) ^ std::rotr(a, 22);
    ...
    std::string sha256(const std::string_view contents) {
        return digest(std::vector<std::uint8_t>(contents.begin(), contents.end()));
    }

    Result<std::string> sha256_file(const std::filesystem::path& path) {
        std::ifstream input(path, std::ios::binary);
        if (!input)
            return std::unexpected(error("cannot open `" + path.string() + "` to calculate SHA-256"));
        ...
        if (!input.eof())
            return std::unexpected(error("cannot read `" + path.string() + "` to calculate SHA-256"));

        return digest(std::move(contents));
    }
}
