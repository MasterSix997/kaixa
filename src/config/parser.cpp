...
namespace kaixa {
    namespace {
        SourceLocation location_of(const toml::node& node, const std::string_view source_name, std::string config_path) {
            const toml::source_region& region = node.source();
            const std::string source = region.path && !region.path->empty() ? *region.path : std::string(source_name);
            return {source, region.begin.line, region.begin.column, std::move(config_path)};
        }

        template <typename T> std::string spell(const T& value) {
        ...
        Value convert_table(const toml::table& table, const std::string_view source_name, std::string path) {
            ...
            for (const auto& [key, child]: table) {
                ...
                entries.push_back({std::string(name), convert(child, source_name, join_config_path(path, name))});
            }
            ...
            return Value::table(std::move(entries), location_of(table, source_name, std::move(path)));
        }
        ...
        Value convert_array(const toml::array& array, const std::string_view source_name, std::string path) {
            ...
            return Value::array(std::move(values), location_of(array, source_name, std::move(path)));
        }
        ...
        Result<Value> convert_result(toml::parse_result result, const std::string_view source_name) {
            if (!result) {
                ...
                return std::unexpected(
                    error_at({source, region.begin.line, region.begin.column, {}}, std::string(result.error().description()))
                );
            }
