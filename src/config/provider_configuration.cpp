        ...
        for (const TableEntry& entry: providers.entries()) {
            if (!is_valid_identifier(entry.key)) {
                return std::unexpected(
                    error_at(entry.value.location(), "`" + entry.key + "` is not a valid provider name")
                );
            }
            ...
            if (!is_valid_identifier(*driver)) {
                return std::unexpected(
                    error_at(definition.location_of("driver"), "`" + *driver + "` is not a valid provider driver name")
                );
            }
            ...
            if (const Value* value = definition.take("default")) {
                ...
                if (!boolean) {
                    return std::unexpected(
                        error_at(definition.location_of("default"), "provider `default` must be a boolean")
                    );
                }
                is_default = *boolean;
            }
