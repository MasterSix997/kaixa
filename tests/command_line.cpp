...
KAIXA_TEST(command_line_parses_package_lifecycle_commands) {
    ...
    constexpr std::array add_arguments = {std::string_view("add"),
        std::string_view("physics"),
    ...
    constexpr std::array update_arguments = {std::string_view("update"), std::string_view("physics"), std::string_view("render")};
...
KAIXA_TEST(command_line_install_selects_a_distribution_prefix) {
    constexpr std::array arguments{std::string_view{"install"},
        std::string_view{"--prefix"},
        std::string_view{"stage"},
        std::string_view{"--profile"},
        std::string_view{"release"}};
    const auto parsed = kaixa::cli::parse_command_line(arguments);
    context.check(parsed.has_value(), "install command parses");
    if (!parsed)
        return;

    const auto* install = std::get_if<kaixa::cli::InstallCommand>(&*parsed);
    context.check(install != nullptr, "install command has its own type");
    if (install) {
        context.check_equal(install->prefix->generic_string(), std::string("stage"), "install prefix is preserved");
        context.check_equal(install->workspace.profile.value_or(""), std::string("release"), "install keeps build profile");
    }
}
...
KAIXA_TEST(command_line_build_keeps_workspace_options) {
    ...
    context.check_equal(command->workspace.path.generic_string(), std::string("project"), "workspace path");
    context.check_equal(command->workspace.profile.value_or(""), std::string("release"), "profile");
...
KAIXA_TEST(command_line_build_lists_or_selects_multiple_targets) {
    ...
    if (selected) {
        ...
        if (command) {
            ...
            context.check_equal(command->workspace.resolver_arguments.size(), std::size_t{2}, "resolver argument scope count");
            context.check_equal(command->workspace.resolver_arguments.front().scope, std::string("configure"), "configure argument scope");
            context.check_equal(command->workspace.resolver_arguments.back().scope, std::string("build"), "build argument scope");
        }
    }

    constexpr std::array list_arguments = {std::string_view("build"), std::string_view("--list")};
...
KAIXA_TEST(command_line_build_rejects_invalid_job_counts) {
    constexpr std::array arguments = {std::string_view("build"), std::string_view("--jobs"), std::string_view("0")};
    const auto parsed = kaixa::cli::parse_command_line(arguments);
    context.check(!parsed.has_value(), "zero jobs are rejected");
    if (!parsed) {
        context.check_contains(parsed.error().message, "positive integer", "job count error explains requirement");
    }
}
...
KAIXA_TEST(command_line_run_distinguishes_one_example_from_example_listing) {
    ...
    constexpr std::array invalid_arguments = {std::string_view("run"), std::string_view("--examples")};
...
KAIXA_TEST(command_line_inspect_targets_accepts_build_configuration) {
    ...
    if (command) {
        context.check(command->mode == kaixa::cli::InspectMode::targets, "inspect target mode is retained");
        context.check_equal(command->workspace.path.generic_string(), std::string("project"), "inspect workspace path");
        context.check_equal(command->workspace.configurations.front(), std::string("clang"), "inspect configuration");
    }
}
...
KAIXA_TEST(command_line_test_keeps_test_options_together) {
    ...
    context.check_equal(command->request.filter.value_or(""), std::string("manifest"), "test filter");
    context.check_equal(command->request.target.value_or(""), std::string("kaixa_tests"), "test target");
    context.check_equal(command->workspace.path.generic_string(), std::string("project"), "test workspace path");
}

KAIXA_TEST(command_line_rejects_extra_positional_arguments) {
    constexpr std::array arguments = {std::string_view("build"), std::string_view("project")};
    const auto parsed = kaixa::cli::parse_command_line(arguments);
    context.check(!parsed.has_value(), "positional path is rejected");
    if (!parsed) {
        context.check_contains(parsed.error().message, "unexpected argument `project`", "specific parsing error");
    }
}
...
KAIXA_TEST(command_line_test_list_composes_with_filter_and_target) {
    ...
    context.check(command->request.mode == kaixa::TestMode::list, "test request uses list mode");
    context.check_equal(command->request.filter.value_or(""), std::string("manifest"), "list filter");
...
KAIXA_TEST(command_line_run_list_is_explicit) {
    ...
    if (command) {
        context.check_equal(command->workspace.path.generic_string(), std::string("project"), "run list workspace");
    }
}
...
KAIXA_TEST(command_line_clean_supports_dry_run_and_all) {
    ...
    context.check_equal(command->workspace.path.generic_string(), std::string("project"), "clean workspace");
}
...
KAIXA_TEST(command_line_config_list_and_path_accept_workspace_paths) {
    ...
    if (listed) {
        ...
        if (command) {
            context.check_equal(command->path.generic_string(), std::string("project"), "config list workspace");
        }
    }
...
KAIXA_TEST(command_line_config_show_composes_named_config_and_overrides) {
    ...
    context.check_equal(command->workspace.configurations.front(), std::string("clang"), "named config selection");
    context.check_equal(command->workspace.profile.value_or(""), std::string("release"), "shown profile override");
    context.check_equal(command->workspace.resolver_arguments.front().scope, std::string("configure"), "shown resolver scope");
}
...
KAIXA_TEST(command_line_workspace_commands_require_the_path_option) {
    ...
    constexpr std::array positional_arguments = {std::string_view("generate"), std::string_view("project")};
